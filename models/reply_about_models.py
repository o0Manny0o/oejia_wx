...
class wx_action_act_article(models.Model):
    ...
    def get_wx_reply(self, openid=None, content=None):
        articles = [article.get_wx_reply(openid, content) for article in self.article_ids]
        return articles

    @api.depends('name')
    def _compute_display_name(self):
        for e in self:
            e.display_name = u'[图文] %s' % e.name
...
class wx_action_act_url(models.Model):
    ...
    def _compute_display_name(self):
        for e in self:
            e.display_name = u'[URL链接] %s' % e.name
